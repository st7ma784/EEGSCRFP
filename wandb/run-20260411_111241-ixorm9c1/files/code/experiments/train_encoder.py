...
class EncoderWithHeads(nn.Module):
    """Encoder + 3 prediction heads with dual-pathway routing.

    The transformer encoder returns two vectors per sample:
        task_vec    — CLS token; trained toward task/alpha discrimination.
        content_vec — mean of M patch outputs; preserves prompt-specific
                      content independently of the task objective.

    Head routing:
        task_head  ← task_vec    (CLS)
        alpha_head ← task_vec    (CLS)
        recon_head ← content_vec (patch mean) — avoids task-objective collapse

    The recon head is a 2-layer MLP so it can extract prompt identity from
    the content_vec even after the transformer has mixed patch representations.

    For linear/mlp encoders task_vec == content_vec (no dual pathway); the
    recon head still uses the same deeper MLP.
    """
    ...
    def __init__(
        ...
        # Initialise alpha head to predict the dataset mean → R²≈0 when
        # objective_alpha_weight=0 rather than wildly negative.
        nn.init.zeros_(self.alpha_head.weight)
        nn.init.constant_(self.alpha_head.bias,
                          float(sum(_ALPHA_VALUES)) / len(_ALPHA_VALUES))
        # 2-layer MLP recon head: content_vec → hidden → d_model
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        """
        enc = self.encoder(x)                           # PatchEncoderOutput
        return {
            "task_logits":  self.task_head(enc.task_vec),             # [B, n_classes]
            "recon_embed":  self.recon_head(enc.content_vec),         # [B, d_model]
            "alpha_pred":   self.alpha_head(enc.task_vec).squeeze(-1), # [B]
        }
